...
# ── Page: Model Info ────────────────────────────────────────────────
    ...
    st.title("📊 Model Info")
    st.markdown(
        "This app uses a 3-hidden-layer **Artificial Neural Network** "
        "(Dense + BatchNorm + Dropout, sigmoid output) trained on NASA "
        "Near-Earth Object data with class-weighting for the ~9:1 imbalance "
        "between hazardous and non-hazardous objects, plus F1-optimized "
        "threshold tuning on a validation set."
    )

    st.subheader("Architecture (best config)")
    st.json(config["best_config"])

    st.subheader("Test-set performance vs. Random Forest baseline")
    ...
    st.caption(f"Decision threshold (F1-optimized on validation set): **{threshold:.2f}**")